...
└── knowledge/                      # 示例知识库（30+ 文件）
    ...
    ├── ai_knowledge/               # AI 行业报告（14 个 PDF）
    │   ├── data_structure.md
    │   ├── 2026年AI_Agent智能体技术发展报告.pdf
    │   ├── OpenAI深度报告：大模型王者，引领AGI之路.pdf
    │   └── ...
    ├── financial_report_data/      # 金融财报数据（6 个 PDF）
    │   ├── data_structure.md
    │   └── ...
    ├── ecommerce_data/             # 电商业务数据（4 个 Excel）
    │   ├── data_structure.md
    │   ├── customers.xlsx
    │   ├── employees.xlsx
    │   ├── inventory.xlsx
    │   └── sales_orders.xlsx
    └── safety_knowledge/           # 安全知识文档（Markdown）
        ├── data_structure.md
        └── ...
```
...
**Skill 执行流程**：
1. 读取 `knowledge/data_structure.md` 识别 `ai_knowledge/` 目录
2. 进入 `ai_knowledge/` 读取子目录索引
3. 定位到 `2026年AI_Agent智能体技术发展报告.pdf`
